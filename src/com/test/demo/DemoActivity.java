...
public class DemoActivity extends Activity implements Callback {
	...
	private void findViews() {
		...
		sound_close = (ImageView) findViewById(R.id.sound_close);
	}
	...
	private void setListeners() {
		...
		imgbtn_Sound.setOnClickListener(Sound_Listener);
		...
		ctrl_1.setOnTouchListener(PTZ_Listener);
		ctrl_2.setOnTouchListener(PTZ_Listener);
		ctrl_3.setOnTouchListener(PTZ_Listener);
		ctrl_4.setOnTouchListener(PTZ_Listener);
		
		ctrl_top.setOnTouchListener(PTZ_Listener);
		ctrl_left.setOnTouchListener(PTZ_Listener);
		ctrl_right.setOnTouchListener(PTZ_Listener);
		ctrl_bottom.setOnTouchListener(PTZ_Listener);

		ctrl_mid.setOnTouchListener(PTZ_Listener);
		mZoomControls.setIsZoomInEnabled(true);  
		mZoomControls.setIsZoomOutEnabled(true);  
		mZoomControls.getChildAt(0).setOnTouchListener(PTZ_Listener);
		mZoomControls.getChildAt(1).setOnTouchListener(PTZ_Listener);
	}
	...
	private boolean startCommant(View v, MotionEvent event){
		try {
			if (m_iLogID < 0) {
			...
			switch (v.getId()) {
			case R.id.ctrl_left:
				Command = PTZCommand.PAN_LEFT;
				break;
			case R.id.ctrl_right:
				Command = PTZCommand.PAN_RIGHT;
				break;
			case R.id.ctrl_top:
				Command = PTZCommand.TILT_UP;
				break;
			case R.id.ctrl_bottom:
				Command = PTZCommand.TILT_DOWN;
				break;
			case R.id.ctrl_1:
				Command = PTZCommand.UP_LEFT;
				break;
			case R.id.ctrl_2:
				Command = PTZCommand.UP_RIGHT;
				break;
			case R.id.ctrl_3:
				Command = PTZCommand.DOWN_LEFT;
				break;
			case R.id.ctrl_4:
				Command = PTZCommand.DOWN_RIGHT;
				break;
			case R.id.ctrl_mid:
				Command = PTZCommand.PAN_AUTO;
				break;
			default:
				break;
			}
			if(mZoomControls.getChildAt(0).equals(v)){
				Command = PTZCommand.ZOOM_OUT;
			}else if(mZoomControls.getChildAt(1).equals(v)){
				Command = PTZCommand.ZOOM_IN;
			}
			if (Command == -1) {
				return false;
			}
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				if (!HCNetSDK.getInstance().NET_DVR_PTZControlWithSpeed_Other(
						m_iLogID, iFirstChannelNo, Command, 0,2)) {
				...
				if (!HCNetSDK.getInstance().NET_DVR_PTZControlWithSpeed_Other(
						m_iLogID, iFirstChannelNo, Command, 1,2)) {
	...
	private void switchSound(boolean open) {
		...
		try {
			...
			if (m_bSoundOn == false && open) {
				...
				sound_close.setVisibility(View.GONE);
				Log.e(TAG, "playSound");
				m_bSoundOn = true;
				...
				sound_close.setVisibility(View.VISIBLE);
				Log.e(TAG, "stopSound");
				m_bSoundOn = false;
			}
		} catch (Exception err) {
		...
		public void onClick(View v) {
			try {
				...
				SimpleDateFormat sDateFormat = new SimpleDateFormat(
						"yyyy-MM-dd-hh:mm:ss");
				String date = sDateFormat.format(new java.util.Date());
				FileOutputStream file = new FileOutputStream(appPath + date
						+ ".bmp");
				Toast.makeText(getApplicationContext(),
						"已保存在SD卡的fch_pic目录下", Toast.LENGTH_LONG).show();
			...
			try {
				...
				if (m_iPlaybackID < 0) {
					...
					if (m_iPlaybackID >= 0) {
						...
						if (!HCNetSDK.getInstance()
								.NET_DVR_PlayBackControl_V40(m_iPlaybackID,
	...
	private void login() {
		try {
			...
			Log.i(TAG,
					"Login sucess ****************************1***************************");
			int code = HCNetSDK.getInstance().NET_DVR_StartVoiceCom_MR_V30(
					m_iLogID, m_oNetDvrDeviceInfoV30.byStartDChan + 2,
					new VoiceDataCallBack() {

						@Override
						public void fVoiceDataCallBack(int arg0, byte[] arg1,
								int arg2, int arg3) {
							// TODO Auto-generated method stub
	...
	private void startPreview(int channel) {
		try {
			...
			ClientInfo.lChannel = iFirstChannelNo; // start channel no + preview
													// channel
			ClientInfo.lLinkMode = channel << 31; // bit 31 -- 0,main
													// stream;1,sub stream
